...
// https://www.googleapis.com/books/v1/volumes?q=search-terms&key=your-${API_KEY}
  ...
  return axios.get(`https://www.googleapis.com/books/v1/volumes?q=search-terms&key=${API_KEY}`)
    .then(res => {
      ...
      if (txt) {
        ...
        cleanItems = cleanItems.filter(item => regex.test(item.title))
      }
...
function getPrice() {
  return utilService.getRandomIntInclusive(0, 200)
}

function get(bookId) {
  return query().then(books => {
    const book = books.find(book => book.googleId === bookId)
    return book
  })
}
